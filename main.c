...
void free_items(Item *item_list, int size) {       //ignores any items that don't have a price listed 
...
double average_price(Item *item_list, int size) {   //adds all prices togeather the divides by the amount of items for avg price
    double total = 0.0;
    for (int i = 0; i < size; i++) {
...
void print_items(Item *item_list, int size) {   //prints every item on the list
    for (int i = 0; i < size; i++) {
        printf("###############\n");
        printf("item name = %s\n", item_list[i].name);
        printf("item sku = %s\n", item_list[i].sku);
...
int main(int argc, char *argv[]) {
    ...
    add_item(item_list, 2.00, "55243", "fruits", "apples", 4);
    ...
    // Search for item by SKU then prints item information
